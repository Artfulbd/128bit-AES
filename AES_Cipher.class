...
public class Main {
    ...
    public static void test_encrypt_decrypt() throws Exception {
        // encrypt "plaintext.txt" -> "crypto.txt"
        String s = readFile("plaintext.txt");
        String res = encrypt("mykey", IV, s);
        PrintWriter writer = new PrintWriter("crypto.txt", "UTF-8");
        writer.print(res);
        writer.close();
        // decrypt "crypto.txt" -> "cleartext.txt"
        s = readFile("crypto.txt");
        res = decrypt("mykey", IV, s);
        writer = new PrintWriter("cleartext.txt", "UTF-8");
        writer.print(res);
        writer.close();
    }
